...
from adv_defence.trainer_adv_example_2 import TrainerAdvExample
...
def main(config):
    ...
    if not config.is_train:
        if config.need_samples:
            ...
            toolkit = TrainerAdvExample(config, None, test_data_loader)
        ...
        trainer = TrainerAdvExample(config, train_data_loader, test_data_loader)
        save_config(config)
        trainer.train()
